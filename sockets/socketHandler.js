...
export const handleSocketConnection = (io, socket) => {
  ...
  socket.on("heartbeat", () => {
    if (socket.userId) {
      redisClient
        .setEx(`user:presence:${socket.userId}`, 60, socket.id)
        .catch(() => {});
    }
  });
  socket.on("join_room", (userId) => {
    ...
    redisClient
      .setEx(`user:presence:${roomName}`, 60, socket.id)
      .catch(() => {});
  });
  ...
  socket.on("confirm_instructions", async ({ sessionId }) => {
    try {
      ...
      if (otherAlreadyRead) {
        ...
        if (activeSession) {
          io.to(activeSession.participants[0].toString()).emit(
            "session_update",
            activeSession,
          );
          io.to(activeSession.participants[1].toString()).emit(
            "session_update",
            activeSession,
          );
        }
      ...
      if (updatedSession) {
        io.to(updatedSession.participants[0].toString()).emit(
          "session_update",
          updatedSession,
        );
        io.to(updatedSession.participants[1].toString()).emit(
          "session_update",
          updatedSession,
        );
      }
  ...
  socket.on("submit_blind_answer", async ({ sessionId, choiceIndex }) => {
    try {
      const sessionInfo = await BlindSession.findById(sessionId).select(
        "participants currentQuestionIndex status currentStage",
      );
      ...
      if (!updatedSession) {
        updatedSession = await BlindSession.findById(sessionId).populate(
          "questions.questionId",
        );
        if (updatedSession) {
          io.to(sessionInfo.participants[0].toString()).emit(
            "session_update",
            updatedSession,
          );
          io.to(sessionInfo.participants[1].toString()).emit(
            "session_update",
            updatedSession,
          );
        }
  ...
  socket.on("proceed_to_next_stage", async ({ sessionId }) => {
    try {
      const sessionInfo = await BlindSession.findById(sessionId).select(
        "participants status currentStage",
      );
      if (
        !sessionInfo ||
      ...
      if (
        updatedSession.stageProgress.u1ReadyNext &&
        updatedSession.stageProgress.u2ReadyNext
      ) {
        let updateQuery = {
          $set: {
            status: "active",
            "stageProgress.u1ReadyNext": false,
            "stageProgress.u2ReadyNext": false,
          },
          $inc: { currentStage: 1, currentQuestionIndex: 1 },
        };
        if (updatedSession.currentStage === 1) {
